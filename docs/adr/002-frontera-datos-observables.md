...
- **Consecuencias**: Un único punto de reemplazo (una línea en `AppModule`) cambia el origen de datos del contacto a `HttpClient` o `environment`. Todos los consumidores usan `subscribe`/`async`; se cumple el pre-flight de "servicios = única frontera de datos". El pipe `currencyFormat` (MXN) y el componente `Breadcrumbs` quedaron en `SharedModule` para ser usados por catálogo y carrito. La ruta de detalle se anidó (`catalog/:categoryId/product/:productId`) para preservar el contexto de categoría en el breadcrumb y recargar una sola vez el `CatalogModule` lazy.
