...
- **Decisión**:
  ...
  - Acceso público vía **Public Development URL** de R2 (`pub-…r2.dev`) como **v1**, porque el frontend vive en GitHub Pages con URL por defecto (sin dominio propio). Node local pasó a **20.19 vía nvm** y `package.json` declara `engines: { node: ">=20.9.0" }` para que Render provisione Node 20+.
- **Consecuencias**:
  ...
  - **No hay eliminación**: reemplazar la imagen de un producto deja los objetos previos en el bucket (huérfanos inofensivos). Un futuro DELETE+GC queda como mejora.
